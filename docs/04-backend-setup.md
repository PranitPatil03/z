...
For implementation-ready guidance across both backend and frontend teams, also use:
...
Contract and docs endpoints:
- `/openapi.json`: autogenerated OpenAPI 3.0.3 spec from mounted routes + Zod validators.
- `/docs`: interactive ReDoc UI powered by `/openapi.json`.
...
- OpenAPI contract endpoint available and current (`/openapi.json`).
