...
export async function generateBudgetNarrativeController(
  request: Request,
  response: Response,
) {
  const typedRequest = request as BudgetNarrativeRequest;
  const validated = (typedRequest.validated?.params || {}) as Record<
    string,
    string
  >;
  ...
  const result = await budgetNarrativeService.generateNarrative(
    costCodeId,
    orgId,
    false,
  );
...
export async function queueBudgetNarrativesController(
  ...
  const typedRequest = request as BudgetNarrativeRequest;
  const validated = (typedRequest.validated?.body || {}) as Record<
    string,
    string
  >;
  const appContext = typedRequest.appContext || {};
  ...
  const result = await budgetNarrativeService.queueNarrativesForProject(
    orgId,
    projectId,
  );
  response.json(result);
