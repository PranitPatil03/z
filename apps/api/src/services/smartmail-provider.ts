...
export function verifySignedStatePayload(
  signedState: string,
  secret: string,
): string {
...
async function fetchGmailMessages(
  input: FetchProviderMessagesInput,
): Promise<NormalizedSmartMailMessage[]> {
  ...
  for (const ref of messageRefs) {
    ...
    const subject =
      headerValue(detail.payload?.headers, "subject") ?? "(no subject)";
    ...
    messages.push({
      ...
      sentAt,
      direction:
        fromEmail === input.accountEmail.toLowerCase() ? "outbound" : "inbound",
      providerMetadata: {
        source: "gmail",
...
async function sendGmailMessage(
  input: SendProviderMessageInput,
): Promise<SendProviderMessageResult> {
  const headers = [
    ...
    ...(input.inReplyToMessageId
      ? [`In-Reply-To: ${normalizeMessageId(input.inReplyToMessageId)}`]
      : []),
    "MIME-Version: 1.0",
