...
const envSchema = z.object({
  ...
  ENCRYPTION_KEY: isProduction
    ? z.string().min(32, "ENCRYPTION_KEY is required in production (min 32 chars)")
    : z.string().optional(),
  ...
  SITE_SNAP_SAFETY_ALERT_MIN_CONFIDENCE_BPS: z.coerce.number().int().min(0).max(10000).optional(),
  BUDGET_AI_PROVIDER: z.enum(["openai", "anthropic", "gemini", "azure-openai"]).optional(),
  BUDGET_AI_MODEL: z.string().optional(),
  BUDGET_DEFAULT_ALERT_THRESHOLD_BPS: z.coerce.number().int().min(0).max(10000).optional(),
